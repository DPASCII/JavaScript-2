...
const shortTitle = movies.filter((x) => x.title.length <= 4);

const longTitle = movies.filter((x) => x.title.length > 4);

const betweenTitle = movies.filter(
  (x) => x.year >= 1980 && x.year <= 1989
).length;

const updatedMovies = movies.map((x) => {
  const currentMovie = x;
  if (x.rating >= 7) {
    currentMovie.tag = "Good";
  }
  if (x.rating >= 4 && x.rating <= 7) {
    currentMovie.tag = "Average";
  }
  if (x.rating < 4) {
    currentMovie.tag = "Bad";
  }
  return currentMovie;
});

const aboveAverageMovies = movies
  .filter((x) => x.rating > 6)
  .map((x) => x.rating);

const specificMovies = movies.filter((x) =>
  x.title
    .toLowerCase()
    .split(" ")
    .filter((y) => y == "surfer" || y == "alien" || y == "benjamin")
).length;

const duplicatedWordsMovies = movies.filter((x) => {
...
const aveRating =
  movies.reduce((acc, curr) => acc + curr.rating, 0) / movies.length;

const goodMovies = 0;
const averageMovies = 0;
const badMovies = 0;

const ratings = updatedMovies.reduce(
  (acc, curr) => {
    acc[curr.tag] += 1;
    return acc;
  },
